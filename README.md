# Git Azterketa: Gida, Komandoak eta Akatsen Konponbidea

Gida honek Git-eko azterketa praktiko batean eskatzen diren kasuistika nagusiak azaltzen ditu, pauso bakoitzean erabili beharreko `gitbash` komandoekin eta ohiko arazoen konponbideekin.
...
## 🔍 Urruneko Errepositorioak Egiaztatzea (`git remote -v`)

`git remote -v` komandoak zure proiektu lokalak lotuta dituen **urruneko biltegiak** (_remotes_, adibidez GitHub, GitLab edo Bitbucket-eko zerbitzariak) zerrendatzeko balio du.

`-v` hizkiak _verbose_ (zehatza) esan nahi du. Komando honek urruneko konexio bakoitzaren izena (normalean `origin` izaten da) eta horri lotutako URLak erakusten dizkizu, bai datuak jasotzeko (`fetch`) eta baita bidaltzeko ere (`push`).

### Adibidea
Zure Git Bash-en exekutatzen duzunean, honen antzeko irteera bat ikusiko duzu:
```bash
origin  https://github.com/AnderUrienGoierri/GI_GIT_azterketa.git (fetch)
origin  https://github.com/AnderUrienGoierri/GI_GIT_azterketa.git (push)
```

### Zertarako da erabilgarria?
1. **Egiaztatzeko:** Zure proiektu lokala GitHub-eko zein biltegi zehatzari konektatuta dagoen ziurtatzeko.
2. **Akatsak konpontzeko:** `git push` edo `git pull` egitean arazorik baduzu, URLak zuzenak diren egiaztatzeko modu azkarrena da.
   _(URLren bat okerra bada, `git remote set-url origin <URL_berria>` erabiliz aldatu dezakezu)._

---

## ⚠️ Ohiko Akatsak eta Konponbideak

### Push Errefusatua (`[rejected] - non-fast-forward` errorea)

Proiektua GitHub-era igotzen saiatzean (`git push`), batzuetan errore hau jasotzen dugu terminalean:

```text
 ! [rejected]        main -> main (non-fast-forward)
error: failed to push some refs to 'https://github.com/AnderUrienGoierri/ML_PHP_jquery_azterketa.git'
...
Errore hau gertatzen da urruneko errepositorioan (GitHub-en) zure ordenagailu lokalean ez dituzun commit-ak daudelako (adibidez, GitHub-en fitxategiren bat zuzenean editatu edo sortu delako, edo taldekide batek aldaketak igo dituelako). Git-ek ez dizu uzten push egiten lehendik dauden aldaketa horiek gainidatzi ez ditzazun.

#### Nola konpondu?
Urruneko aldaketak zure tokiko adarrekin sinkronizatu behar dituzu zure tokiko commits-ak galdu gabe. Horretarako, exekutatu komando hau:
...
Komando honek:
1. GitHub-eko commit berriak deskargatzen ditu.
2. Zure tokiko commit-ak aldi baterako albo batera uzten ditu.
3. GitHub-etik ekarritako commit-ak aplikatzen ditu.
4. Zure tokiko commit-ak haien gainean ezartzen ditu berriro (rebase).

Ondoren, arazorik gabe igo ditzakezu aldaketak GitHub-era:
...
| `git remote -v`                 | Zure errepositorio lokalari lotutako urruneko errepositorioen URL-ak erakusten ditu (fetch eta push).                       |
| `git pull origin <branch> --rebase` | Urruneko adarraren aldaketak ekartzen ditu tokiko adarreko commits-ak haien gainean berrantolatuz.                          |
